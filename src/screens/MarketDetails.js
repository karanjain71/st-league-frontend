...
import { NativeBaseProvider, Box, Text, FlatList, HStack, VStack, Spacer, Icon } from 'native-base';
import Footer from '../components/Footer';
import { getAllStocks, getTopGainers, getTopLosers } from '../helpers/backend_helper';
...
const MarketDetails = ({navigation,}) => {
  ...
  const Market = () => {
    ...
    return (<FlatList data={data} renderItem={({
      ...
      borderColor: "muted.50"
    }} borderColor="muted.800" pl={5} pr={5} pb={2} marginY={2}>
          ...
          </Box>} keyExtractor={item => item.CompanyName} />
    )};

    const TopGainers = () => {

      return (<FlatList data={topGainersData} renderItem={({
        item
      }) => <Box borderBottomWidth="0.2" _dark={{
        borderColor: "muted.50"
      }} borderColor="muted.800" pl={5} pr={5} pb={2} marginY={2}>
      ...
      const TopLosers = () => {
        ...
        return (<FlatList data={topLosersData} renderItem={({
          item
        }) => <Box borderBottomWidth="0.2" _dark={{
          borderColor: "muted.50"
        }} borderColor="muted.800" pl={5} pr={5} pb={2} marginY={2}>
